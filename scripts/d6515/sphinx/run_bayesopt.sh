...
function run() {
	...
	for percentile in ${PERCENTILE}; do
		...
		rsync --mkpath -avz ${newdir}/* don:/home/handong/cloudlab/d6515/sphinx/linux_static/${newdir}/
	    done
	done
    done
}
...
function runMult() {
    NTRIALS=40 MQPS=10 PERCENTILE=99 LATENCY=12000000 run
    NTRIALS=40 MQPS=8 PERCENTILE=99 LATENCY=12000000 run
    NTRIALS=40 MQPS=4 PERCENTILE=99 LATENCY=12000000 run
    
    NTRIALS=40 MQPS=10 PERCENTILE=90 LATENCY=6000000 run
    NTRIALS=40 MQPS=8 PERCENTILE=90 LATENCY=6000000 run
    NTRIALS=40 MQPS=4 PERCENTILE=90 LATENCY=6000000 run
}

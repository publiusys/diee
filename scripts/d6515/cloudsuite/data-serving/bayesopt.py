...
def runDataServer(itr, dvfs):
    print(f"MITR={itr} MDVFS={dvfs} MQPS={TARGET_QPS} ./run_data-server.sh runOneStatic")
    p1 = runLocalCommand(f"MITR={itr} MDVFS={dvfs} MQPS={TARGET_QPS} ./run_data-server.sh runOneStatic")
    ...
    if os.path.isfile(server_rapl) and os.path.isfile(client1lats):
        ...
        with open(client1lats) as file:
            for line in file:
                ...
                if "[READ]" in line.rstrip():
                    if "95th" in line.rstrip():
                        LATENCIES1["95"] = float(line.rstrip().split(",")[2])
                    if "99th" in line.rstrip():
                        LATENCIES1["99"] = float(line.rstrip().split(",")[2])
                if "[UPDATE]" in line.rstrip():
                    if "95th" in line.rstrip():
                        LATENCIES2["95"] = float(line.rstrip().split(",")[2])
                    if "99th" in line.rstrip():
                        LATENCIES2["99"] = float(line.rstrip().split(",")[2])
        print(LATENCIES1, LATENCIES2)

        meanlatency = mean([LATENCIES1[percentile_target], LATENCIES2[percentile_target]])
        return joules, meanlatency
    else:
        return -1.0, -1.0


def data_server_eval_func(params):
    global lat_target
    ...
    joules, meanlatency = runDataServer(itr, dvfs)
    if joules == -1.0:
        sys.exit("joules == -1.0")
...
def perform_bayesopt(metric = 'read_99th_mean', minimize = True, ntrials=30):
    ...
    if MINLATENCY == True:        
        best_params, values, exp, model = optimize(parameters=search_space,
                                                   evaluation_function=lambda params: data_server_eval_func(params),
                                                   experiment_name=f'data_server_discrete',
                                                   objective_name='data_server',
                                                   minimize=minimize,

...
function run() {
	...
	for percentile in ${PERCENTILE}; do
		...
		rsync --mkpath -avz ${newdir}/* don:/home/handong/cloudlab/rs620/sphinx/linux_static/${newdir}/
	    done
	done
    done
}
...
function runMult() {
    NTRIALS=40 MQPS=5 PERCENTILE=99 LATENCY=12000000 ./run_bayesopt.sh run
    ...
    NTRIALS=40 MQPS=5 PERCENTILE=90 LATENCY=6000000 ./run_bayesopt.sh run

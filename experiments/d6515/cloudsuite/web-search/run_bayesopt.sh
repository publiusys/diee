...
function run() {
	...
	for percentile in ${PERCENTILE}; do
		...
		rsync --mkpath -avz ${newdir}/* don:/home/handong/cloudlab/d6515/cloudsuite/web_search/linux_static/${newdir}/
	    done
	done
    done
}
...
function runMult() {
    NTRIALS=40 MQPS=2500 PERCENTILE=99 LATENCY=200000 run
    NTRIALS=40 MQPS=1500 PERCENTILE=99 LATENCY=200000 run

    NTRIALS=40 MQPS=2500 PERCENTILE=90 LATENCY=100000 run
    NTRIALS=40 MQPS=1500 PERCENTILE=90 LATENCY=100000 run
}

...
function run() {
	...
	for percentile in ${PERCENTILE}; do
		...
		rsync --mkpath -avz ${newdir}/* don:/home/handong/cloudlab/d6515/xapian/linux_static/${newdir}/
	    done
	done
    done
}
...
function runMult() {
    ...
    NTRIALS=40 MQPS=2000 PERCENTILE=99 LATENCY=10000 run
    NTRIALS=40 MQPS=3000 PERCENTILE=99 LATENCY=10000 run

    NTRIALS=40 MQPS=1000 PERCENTILE=90 LATENCY=5000 run
    NTRIALS=40 MQPS=2000 PERCENTILE=90 LATENCY=5000 run
    NTRIALS=40 MQPS=3000 PERCENTILE=90 LATENCY=5000 run
}

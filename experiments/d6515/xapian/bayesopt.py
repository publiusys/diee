...
def runXapian(itr, dvfs):
    p1 = runLocalCommand(f"MITR={itr} MDVFS={dvfs} MQPS={TARGET_QPS} ./run_xapian.sh runOneStatic")
...
def xapian_eval_func(params):
    ...
    joules, meanlatency, maxlatency = runXapian(itr, dvfs)
    if joules == -1.0:
        sys.exit("joules == -1.0")
...
def perform_bayesopt(metric = 'read_99th_mean', minimize = True, ntrials=30):
    ...
    if MINLATENCY == True:        
        best_params, values, exp, model = optimize(parameters=search_space,
                                                   evaluation_function=lambda params: xapian_eval_func(params),
                                                   experiment_name=f'xapian_discrete',
                                                   objective_name='xapian',
                                                   minimize=minimize,
